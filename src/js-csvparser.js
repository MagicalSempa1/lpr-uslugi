...
function parse(data, options) {
	...
	loop1: for (let start = 0, end = 0, rows = 0, columns = 0; ; end++) {
		...
		if (data[end] === options.delimiter) {
			// field
			columns++;
			if (columns === options.maxColumns.numberOfColumns) {
				// max columns reached
				if (options.maxColumns.cutRemaining) {
					// find next row
					row.push(data.substring(start, end));
		...
		if (startOrNewLine(end) && data[end] === options.comment) {
			// comment
			end = nextLineOrEOF(end); // search for next line or EOF
		...
		if (data[end] === '"') {
			...
			if (startOrNewLine(end) || data[end - 1] === options.delimiter) {
				...
				loop2: for (; ; end++) {
					...
					if (data[end] === '"') {
						...
						if (data[end + 1] === options.delimiter) {
							// field
							columns++;
							if (columns === options.maxColumns.numberOfColumns) {
								// max columns reached
								if (options.maxColumns.cutRemaining) {
									...
									row.push(
										data
											.substring(start, data[end - 1] === '"' ? end - 1 : end)
											.replace(/""/g, '"')
											.replace(/","/g, ',')
									);
								}
				...
				if (data[end + 1] === options.delimiter || end === len - 1) {
...
function detectLineEnding(data) {
	...
	let arr = [
		{ type: '\n', data: n, length: n.length - rn.length },
		{ type: '\r', data: r, length: r.length - rn.length },
